...
let targetDir, coreGit, deployerGit;

const handleCreate = async (name, dir, workspace, core, deployer) => {
  if (workspace && core && deployer) {
    targetDir = path.resolve(workspace, name);
    coreGit = vineGitRepo("core", core);
    deployerGit = vineGitRepo("deployer", deployer);
  } else {
    log.logo();
    // get target path and print path
    targetDir = path.resolve(dir, name);
    log.info("Vine process will create a new project at " + targetDir);
    // download core and deployer by Git
    const ans = await inquirer.prompt(promptList);
    coreGit = vineGitRepo("core", ans.core);
    deployerGit = vineGitRepo("deployer", ans.deployer);
  }
